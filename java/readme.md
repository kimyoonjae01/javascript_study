...
### 데이터 구분하는 쉼표와 연결연산자(+)
* `console.log("a는", a)`
* 위 쉼표 수굽ㄴ은 문자 "a는"과 변수 a를 각각 다르게 구분하여 필요 시 문자와 변수를 다르게 처리할 수 있다.
* `console.log("a는"+ a)`
...
* 배열의 데이터 번호 == index
* `배열명[index]`
* 배열의 `iength` 속성은 index와 다르게 1부터 인식한다.
* `배열.length` `객체, 속성` 
### 배열의 추가 속성
...
--------
## 객체 object 
* `변수 생성 키워드 객체 먕 = {속성:값, 속성:값,}`
* 객체의 속성은 **key** 라고도 부른다
* 속성(key) 명은 그 속성이 가진 값에 어울리는 의미적인 이름으로 작성한다.(정해진 키워드가 아님)
* 객체 속성에 2개 이상의 값을 설장할 때는 배열을 이용해야 한다,
* `객체명 = {key:['배열 첫번째 값', '배열 두번째 값'],}`
## 객체와 배열
* 키:값 구조로 된 객체를 큰 분류로 2개이상 제작할 때는 배열을 이용해야 한다,
